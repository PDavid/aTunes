...
public class AutoSetTagFromFileNamePatternAction extends ActionOverSelectedObjects<AudioFile> {
    ...
    public AutoSetTagFromFileNamePatternAction() {
        super(StringUtils.getString(LanguageTool.getString("AUTO_SET_TAG_FROM_FILE_NAME_PATTERN"), "..."), AudioFile.class);
        putValue(SHORT_DESCRIPTION, LanguageTool.getString("AUTO_SET_TAG_FROM_FILE_NAME_PATTERN"));
    }

    @Override
    protected void performAction(List<AudioFile> objects) {
        // Show pattern input dialog
        PatternInputDialog inputDialog = new PatternInputDialog(VisualHandler.getInstance().getFrame().getFrame(), false);
