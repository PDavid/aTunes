...
public final class NavigationHandler extends AbstractHandler implements
	...
	applyNavigationTableVisibility(stateNavigation.isShowNavigationTree()
		&& stateNavigation.isShowNavigationTable());
	...
	return getNavigationController().getAudioObjectInNavigationTable(
		navigationTable.getSelectedRow());
    }
	...
	for (int i = 0; i < navigationTable.getSelectedRows().length; i++) {
	    result.add(getNavigationController()
		    .getAudioObjectInNavigationTable(i));
	}
	return result;
    }
