...
public final class DeviceSearchableObject extends AbstractCommonAudioFileSearchableObject {
    ...
    public synchronized FSDirectory getIndexDirectory() throws IOException {
        if (indexDirectory == null) {
            indexDirectory = new SimpleFSDirectory(new File(StringUtils.getString(SystemProperties.getUserConfigFolder(Kernel.isDebug()), "/", Constants.DEVICE_INDEX_DIR)));
        }
    ...
    public List<AudioObject> getSearchResult(List<RawSearchResult> rawSearchResults) {
        ...
        for (RawSearchResult rawSearchResult : rawSearchResults) {
        	LocalAudioObject audioFile = DeviceHandler.getInstance().getFileIfLoaded(rawSearchResult.getDocument().get("url"));
            if (audioFile != null) {
