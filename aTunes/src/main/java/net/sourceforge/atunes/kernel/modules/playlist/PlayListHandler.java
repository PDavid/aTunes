...
public final class PlayListHandler extends AbstractHandler implements
	...
	newPlayList(playListNameCreator.getNameForPlaylist(playListsContainer,
		null), audioObjects);
    }
	...
	addNewPlayList(nameOfNewPlayList,
		playListCreator.getNewPlayList(nameOfNewPlayList, audioObjects));
	playListsChanged();
    }
	...
	return playListTabController.getPlayListName(playListTabController
	...
	if (isActivePlayListVisible() && playList.size() == audioObjects.size()) {
	    ...
	    playListEventListeners.selectedAudioObjectHasChanged(audioObjects
	...
	playListPersistor.persistPlayLists(listOfPlayListsCreator
		.getListOfPlayLists(playListsContainer));
    }
	...
	for (IPlayList playlist : listOfPlayLists.getPlayLists()) {
	    addNewPlayList(playListNameCreator.getNameForPlaylist(
		    playListsContainer, playlist), playlist);
	}
