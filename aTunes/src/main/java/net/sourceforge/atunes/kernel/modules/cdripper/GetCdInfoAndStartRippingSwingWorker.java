 ...
 * 
 */
public final class GetCdInfoAndStartRippingSwingWorker extends
	...
	CdRipper ripper = beanFactory.getBean(CdRipper.class);
	ripper.setNoCdListener(new NoCdListener() {
	    @Override
	    public void noCd() {
		Logger.error("No cd inserted");
		GetCdInfoAndStartRippingSwingWorker.this.ripperHandler
			.setInterrupted(true);
	...
	try {
		...
		this.ripperHandler.getRipCdDialogController()
			.showCdInfo(cdInfo);
		if (!this.ripperHandler.getRipCdDialogController()
			.isCancelled()) {
