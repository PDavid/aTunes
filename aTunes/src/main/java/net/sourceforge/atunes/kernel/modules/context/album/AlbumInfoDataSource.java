...
public class AlbumInfoDataSource implements ContextInformationDataSource {
	...
	private AlbumInfo getAlbumInfo(AudioObject audioObject) {
        ...
        if (album == null) {
        	...
        	if (!audioObject.getArtist().equals(LanguageTool.getString("UNKNOWN_ARTIST"))) {
        		// Get 
        		AlbumListInfo albumList = LastFmService.getInstance().getAlbumList(audioObject.getArtist(), ApplicationState.getInstance().isHideVariousArtistsAlbums(), ApplicationState.getInstance().getMinimumSongNumberPerAlbum());
        		if (albumList != null) {
    ...
    /**
     * Saves an image related to an audio file from a web service in the folder where audio file is 
     * @param img
     * @param file
     */
