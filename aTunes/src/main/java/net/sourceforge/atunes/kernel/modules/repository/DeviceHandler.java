...
public final class DeviceHandler extends AbstractHandler implements
	...
	caseSensitiveTrees = stateRepository
	...
	if (caseSensitiveTrees != stateRepository
		.isKeyAlwaysCaseSensitiveInRepositoryStructure()) {
	    caseSensitiveTrees = stateRepository
		    .isKeyAlwaysCaseSensitiveInRepositoryStructure();
	    refreshDevice();
	...
	if (isDefaultDeviceLocationConfigured(stateDevice)
		&& !deviceMonitor.isMonitorRunning()) {
	    deviceMonitor.startMonitor();
	} else if (!isDefaultDeviceLocationConfigured(stateDevice)
		&& deviceMonitor.isMonitorRunning()) {
	...
	long deviceFreeSpace = deviceRepository.getRepositoryFolders().get(0)
		.getFreeSpace();
	if (leaveFree >= 0) {
	...
	return deviceRepository.getRepositoryFolders().get(0).getFreeSpace()
    ...
    private void runProcessToCopyFiles(
	    ...
	    final IProcessListener<List<File>> listener) {
	final ILocalAudioObjectTransferProcess process = (ILocalAudioObjectTransferProcess) processFactory
	...
	process.setDestination(net.sourceforge.atunes.utils.FileUtils
		.getPath(deviceRepository.getRepositoryFolders().get(0)));
	...
	beanFactory.getBean(IStateHandler.class).persistDeviceCache(deviceId,
		deviceRepository);
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		beanFactory.getBean(RefreshDeviceAction.class)
			.setEnabled(false);
		beanFactory.getBean(DisconnectDeviceAction.class).setEnabled(
			false);
		beanFactory.getBean(SynchronizeDeviceWithPlayListAction.class)
			.setEnabled(false);
		beanFactory.getBean(CopyPlayListToDeviceAction.class)
			.setEnabled(false);
		getFrame().showDeviceInfo(false);
		dialogFactory.newDialog(IMessageDialog.class).showMessage(
			I18nUtils.getString("DEVICE_DISCONNECTION_DETECTED"));
	    }
	});
	...
	if (isDeviceConnected()) {
	    // Persist device metadata
	    beanFactory.getBean(IStateHandler.class).persistDeviceCache(
		    deviceId, deviceRepository);
	}
    }
	...
	if (loader != null) {
	    ...
	    navigationHandler.setNavigationView(deviceNavigationView.getClass()
		    .getName());
	}
    }
	...
	if (isDeviceConnected()) {
	    ...
	    deviceRepository = new Repository(
		    oldDeviceRepository.getRepositoryFolders(), stateRepository);
	    currentLoader = beanFactory.getBean(RepositoryRefreshLoader.class);
	    currentLoader.setRepositoryLoaderListener(this);
	    currentLoader.start(new RepositoryTransaction(deviceRepository,
		    null), oldDeviceRepository.getRepositoryFolders(),
		    oldDeviceRepository, deviceRepository);
	}
    }
	...
	if (deviceId != null) {
	    deviceRepository = beanFactory.getBean(IStateHandler.class)
		    .retrieveDeviceCache(deviceId);
	    if (deviceRepository != null) {
		refreshDevice();
		return;
	    }
	...
	currentLoader.setRepositoryLoaderListener(this);
	currentLoader.start(new RepositoryTransaction(deviceRepository, null),
		folders, null, deviceRepository);
    }
	...
	return deviceRepository == null ? null : deviceRepository
		.getFile(fileName);
    }
	...
	try {
	    File f = new File(StringUtils.getString(devicePath, getOsManager()
	...
	String artist = af.getAlbumArtist(unknownObjectChecker) != null
		&& !af.getAlbumArtist(unknownObjectChecker).trim().equals("") ? af
		.getAlbumArtist(unknownObjectChecker) : af
	...
	return deviceRepository != null ? net.sourceforge.atunes.utils.FileUtils
		.getPath(deviceRepository.getRepositoryFolders().get(0)) : null;
    }
