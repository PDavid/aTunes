...
final class CustomSearchController extends
	...
	getComponentControlled().getAdvancedSearchCheckBox().setSelected(
		stateCore.isEnableAdvancedSearch());
	...
	if (searchableObjects != null) {
	    ...
	    ISearchableObject selectedSearchableObject = searchableObjects
	...
	for (String translatedAttr : translatedAttributesList) {
	    translatedQuery = translatedQuery.replaceAll(translatedAttr,
		    translatedAttributes.get(translatedAttr));
	}
	...
	if (searchableObjects != null) {
	    ...
	    try {
		// Invoke query
		List<IAudioObject> result = searchHandler.search(
		...
		if (result.isEmpty()) {
		    ...
		    // Show result
		    searchHandler.showSearchResults(selectedSearchableObject,
			    result);
		}
	...
	getComponentControlled().getComplexRulesRemoveButton().setEnabled(
		!enable);
	...
	public void actionPerformed(final ActionEvent e) {
	    ...
	    stateCore.setEnableAdvancedSearch(getComponentControlled()
		    .getAdvancedSearchCheckBox().isSelected());
	}
    }
    ...
    private static class TranslatedAttributesList implements
