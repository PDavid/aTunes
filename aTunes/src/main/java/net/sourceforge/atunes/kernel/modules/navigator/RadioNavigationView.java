...
public final class RadioNavigationView extends AbstractNavigationView {
	 ...
	 */
	public void setRadioNavigationColumnSet(final IColumnSet radioNavigationColumnSet) {
	...
	public JPopupMenu getTreePopupMenu() {
		if (radioTreeMenu == null) {
			...
			AbstractActionOverSelectedObjects<IAudioObject> addToPlayListAction = beanFactory.getBean("addToPlayListFromRadioNavigationView", AddToPlayListAction.class);
			addToPlayListAction.setAudioObjectsSource(this);
			radioTreeMenu.add(addToPlayListAction);

			SetAsPlayListAction setAsPlayListAction = beanFactory.getBean("setAsPlaylistFromRadioNavigationView", SetAsPlayListAction.class);
			...
			AbstractActionOverSelectedObjects<IRadio> addFavoriteRadioAction = beanFactory.getBean(AddFavoriteRadioAction.class);
			addFavoriteRadioAction.setAudioObjectsSource(this);
			radioTreeMenu.add(addFavoriteRadioAction);

			EditRadioAction editRadioAction = beanFactory.getBean(EditRadioAction.class);
			editRadioAction.setAudioObjectsSource(this);
			radioTreeMenu.add(editRadioAction);

			radioTreeMenu.add(beanFactory.getBean(RenameRadioLabelAction.class));

			RemoveRadioAction removeRadioAction = beanFactory.getBean(RemoveRadioAction.class);
	...
	public JPopupMenu getTablePopupMenu() {
		if (radioTableMenu == null) {
			...
			AbstractActionOverSelectedObjects<IAudioObject> addToPlayListAfterCurrentAudioObjectAction = beanFactory.getBean("addToPlayListAfterCurrentAudioObjectFromRadioNavigationView", AddToPlayListAfterCurrentAudioObjectAction.class);
			...
			radioTableMenu.add(new JMenuItem(beanFactory.getBean(ShowNavigatorTableItemInfoAction.class)));
			radioTableMenu.add(new JSeparator());
			AbstractActionOverSelectedObjects<IRadio> addFavoriteRadioAction = beanFactory.getBean(AddFavoriteRadioAction.class);
			addFavoriteRadioAction.setAudioObjectsSource(this);
	...
	@Override
	public List<IAudioObject> getAudioObjectForTreeNode(final DefaultMutableTreeNode node, final ViewMode viewMode, final String treeFilter) {
	 ...
	 */
	private void addRadioNodes(final List<IRadio> radios, final List<IRadio> presetRadios, final DefaultMutableTreeNode root, final String currentFilter, final boolean showAllStations, final List<ITreeObject<? extends IAudioObject>> objectsExpanded, final List<ITreeObject<? extends IAudioObject>> objectsSelected, final List<DefaultMutableTreeNode> nodesToExpand, final List<DefaultMutableTreeNode> nodesToSelect) {
		if (radios == null) {
			return;
		}
	...
	private void addRadioNodes(final List<IRadio> radios,
			final List<IRadio> presetRadios,
			...
			final Map<String, DefaultMutableTreeNode> radioGroups,
			final Map<String, DefaultMutableTreeNode> radioGroupNoLabel) {
	...
	private void createRadioNode(final List<IRadio> presetRadios,
			...
			final List<DefaultMutableTreeNode> nodesToSelect,
			final Map<String, DefaultMutableTreeNode> radioGroups,
			final Map<String, DefaultMutableTreeNode> radioGroupNoLabel, final IRadio r) {
	...
	private void markNodeAsExpanded(
			final List<ITreeObject<? extends IAudioObject>> objectsExpanded,
			final List<DefaultMutableTreeNode> nodesToExpand,
			final DefaultMutableTreeNode radioNode) {
