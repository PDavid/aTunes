...
public final class NavigationTableMouseListener extends MouseAdapter {
	...
	if (GuiUtils.isSecondaryMouseButton(event)) {
	    ...
	    if (!found) {
		navigationTable.getSelectionModel().setSelectionInterval(
			selected, selected);
	    }

	    // Enable or disable actions of popup
	    currentView.updateTablePopupMenuWithTableSelection(navigationTable,
		    event);

	    // Show popup
	    currentView.getTablePopupMenu()
		    .show(controller.getPopupMenuCaller(), event.getX(),
			    event.getY());
	} else {
	    if (event.getClickCount() == 2) {
		int[] selRow = navigationTable.getSelectedRows();
		List<IAudioObject> songs = ((NavigationTableModel) navigationTable
