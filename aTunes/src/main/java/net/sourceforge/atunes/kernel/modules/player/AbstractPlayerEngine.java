...
public abstract class AbstractPlayerEngine implements IPlayerEngine {
    ...
    public void playAudioObjectInPlayListPositionOfActivePlayList(
	    final int position) {
	playAudioObjectFromPlayList(position,
		playListHandler.getActivePlayList());
    }
	...
	try {
	    boolean activateFadeAway = userStopped
		    && statePlayer.isUseFadeAway() && !paused;
	    if (paused) {
		paused = false;
	    }
	...
	switchPlaybackTo(playListHandler.getNextAudioObject(), true,
		audioObjectFinished);
    }
	...
	playerHandler.setCurrentAudioObjectTimePlayed(actualPlayedTime,
		currentAudioObjectLength);
	...
	if (audioObject.exists()) {
		...
		PlayAudioObjectRunnable r = new PlayAudioObjectRunnable(this,
			audioObject, frame, temporalDiskStorage);
	    ...
	    playListHandler.checkPlayListsItems();
	}
    }
	...
	if (audioObject instanceof ILocalAudioObject
		&& statePlayer.isCacheFilesBeforePlaying()) {
	    ...
	    if (tempFile != null) {
		audioObjectToPlay = localAudioObjectFactory
	...
	if (audioObjectToSwitchTo != null) {
		...
		playListEventListeners
			.selectedAudioObjectHasChanged(playListHandler
	...
	float perCent = (((float) playerHandler
		.getCurrentAudioObjectPlayedTime()) / playerHandler
