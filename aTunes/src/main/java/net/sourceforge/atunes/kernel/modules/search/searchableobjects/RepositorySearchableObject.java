...
public final class RepositorySearchableObject extends AbstractCommonAudioFileSearchableObject {
    ...
    public FSDirectory getIndexDirectory() throws IOException {
        if (indexDirectory == null) {
            indexDirectory = new SimpleFSDirectory(new File(StringUtils.getString(OsManager.getUserConfigFolder(Kernel.isDebug()), "/", Constants.REPOSITORY_INDEX_DIR)));
        }
    ...
    public List<AudioObject> getSearchResult(List<RawSearchResult> rawSearchResults) {
        ...
        for (RawSearchResult rawSearchResult : rawSearchResults) {
        	LocalAudioObject audioFile = RepositoryHandler.getInstance().getFileIfLoaded(rawSearchResult.getDocument().get("url"));
            if (audioFile != null) {
