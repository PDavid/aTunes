...
public class RssSmallImageIcon extends CachedIconFactory {
	...
	private Area getSmallIconArea(int size) {
		int margin = 0;
		int arc = size / 4;
		arc = arc > 4 ? arc : 4;
		...
		a.subtract(RssImageIcon.getArc(pathWidth, cpX, cpY, startX, startY, endX, endY));
		a.subtract(RssImageIcon.getArc(pathWidth, cpX - deltaArc2, cpY + deltaArc2, startX, startY + deltaArc2, endX - deltaArc2, endY));
		a.subtract(RssImageIcon.getArc(pathWidth, cpX - deltaArc3, cpY + deltaArc3, startX, startY + deltaArc3, endX - deltaArc3, endY));
